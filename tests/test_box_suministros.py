...
def usar(objetivo, precio=None):
    return usar_suministro(
        GUILD,
        USUARIO,
        objetivo,
...
def test_vida_restaura_al_maximo(base_datos_limpia):
    dar_dinero(5000)
    obtener_equipo(GUILD, USUARIO)  # crea la fila de equipo
    actualizar_equipo(GUILD, USUARIO, vida=10)

    estado, saldo = usar("vida")

    assert estado == "comprado"
    assert saldo == 5000 - PRECIOS["vida"]
    assert obtener_equipo(GUILD, USUARIO)["vida"] == (
        obtener_equipo(GUILD, USUARIO)["vida_maxima"]
    )


def test_vida_llena_no_cobra(base_datos_limpia):
    dar_dinero(5000)
    ...
    assert estado == "lleno"
    assert saldo == 5000
    assert obtener_saldo(GUILD, USUARIO)[1] == 5000
...
def test_cansancio_restaura_al_maximo(base_datos_limpia):
    dar_dinero(5000)
    obtener_equipo(GUILD, USUARIO)  # crea la fila de equipo
    actualizar_equipo(GUILD, USUARIO, cansancio=5)

    estado, saldo = usar("cansancio")

    assert estado == "comprado"
    equipo = obtener_equipo(GUILD, USUARIO)
    assert equipo["cansancio"] == equipo["cansancio_maximo"]


def test_cansancio_lleno_no_cobra(base_datos_limpia):
    dar_dinero(5000)

    estado, _ = usar("cansancio")
...
def test_defensa_repara_hasta_el_maximo(base_datos_limpia):
    dar_dinero(5000)
    # La defensa arranca en 1 con máximo 22.
    actualizar_equipo(GUILD, USUARIO, defensa=3)

    estado, _ = usar("defensa")
...
def test_defensa_ya_reparada_no_cobra(base_datos_limpia):
    dar_dinero(5000)
    obtener_equipo(GUILD, USUARIO)  # crea la fila de equipo
    actualizar_equipo(GUILD, USUARIO, defensa=22)

    estado, saldo = usar("defensa")
...
def test_lesion_cura_y_reinicia_probabilidad(base_datos_limpia):
    dar_dinero(70000)
    lesionar()
    admin_modificar_probabilidad_lesion(GUILD, USUARIO, 40.0)

    estado, _ = usar("lesion")

    assert estado == "comprado"

    probabilidad, lesionado_hasta = obtener_estado_box(GUILD, USUARIO)
    assert probabilidad == 0.0
    assert lesionado_hasta is None


def test_lesion_sin_lesion_ni_probabilidad_no_cobra(base_datos_limpia):
    dar_dinero(70000)

    estado, saldo = usar("lesion")

    assert estado == "sin_lesion"
    assert saldo == 70000


def test_lesion_con_probabilidad_sin_lesion_cura_probabilidad(
    base_datos_limpia,
):
    """Si no hay lesión activa pero sí probabilidad, el botiquín la reinicia."""

    dar_dinero(70000)
    admin_modificar_probabilidad_lesion(GUILD, USUARIO, 12.0)

    estado, _ = usar("lesion")

    assert estado == "comprado"
    assert obtener_estado_box(GUILD, USUARIO)[0] == 0.0


def test_lesion_sin_dinero_informa(base_datos_limpia):
    lesionar()
...
def test_objetivo_invalido(base_datos_limpia):
    dar_dinero(5000)

    estado, saldo = usar("ataque", precio=1500)
...
def test_comando_suministro_vida_restaura(cog):
    dar_dinero(5000)
    obtener_equipo(GUILD, USUARIO)
    actualizar_equipo(GUILD, USUARIO, vida=4)

    interaccion = InteraccionFalsa(GUILD, USUARIO)
    llamar(cog, "suministro", interaccion, Choice("vida"))
...
def test_comando_suministro_vida_llena_avisa(cog):
    dar_dinero(5000)
    obtener_equipo(GUILD, USUARIO)
...
def test_comando_suministro_defensa_repara(cog):
    dar_dinero(5000)
    obtener_equipo(GUILD, USUARIO)

    interaccion = InteraccionFalsa(GUILD, USUARIO)
    llamar(cog, "suministro", interaccion, Choice("defensa"))

    assert "Servicio de reparación" in interaccion.texto
    assert "🛡️ Defensa: **22/22**" in interaccion.texto


def test_comando_suministro_tipo_invalido(cog):
    interaccion = InteraccionFalsa(GUILD, USUARIO)
    llamar(cog, "suministro", interaccion, Choice("no_existe"))
...
def test_boton_suministro_abre_el_menu_efimero():
    ...
    asyncio.new_event_loop().run_until_complete(
        boton_suministro.callback(interaccion)
    )
...
def test_selector_de_suministro_aplica_y_se_deshabilita():
    from commands.box.tienda import VistaSuministro

    dar_dinero(5000)
    obtener_equipo(GUILD, USUARIO)
    actualizar_equipo(GUILD, USUARIO, vida=7)
...
def test_selector_rechaza_a_un_usuario_que_no_es_el_dueño():
    from commands.box.tienda import VistaSuministro
