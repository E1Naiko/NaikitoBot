...
async def registrar_servicio(hora, minuto=0, user_id=USUARIO, nombre="Tester"):
    return await registrar_madrugue(
        GUILD,
        user_id,
        nombre,
...
async def test_ayuda_funciona_por_mensaje_directo(cog):
    interaccion = InteraccionFalsa(GUILD, USUARIO, en_servidor=False)
...
async def test_extension_registra_comandos_de_primer_nivel(base_datos_limpia):
...
async def test_madrugue_registra(cog, monkeypatch):
    fijar_hora(monkeypatch, 5, 45)
    interaccion = InteraccionFalsa(GUILD, USUARIO)

    await llamar(cog, "madrugue", interaccion)
...
async def test_madrugue_dos_veces_informa(cog, monkeypatch):
    fijar_hora(monkeypatch, 5, 45)
    await llamar(cog, "madrugue", InteraccionFalsa(GUILD, USUARIO))
    interaccion = InteraccionFalsa(GUILD, USUARIO)
...
async def test_madrugue_fuera_de_horario_informa(cog, monkeypatch):
...
async def test_stats_muestra_puntos_y_racha(cog):
    await registrar_servicio(5, 45)
    interaccion = InteraccionFalsa(GUILD, USUARIO)
...
async def test_top_sin_registros_informa(cog):
    interaccion = InteraccionFalsa(GUILD, USUARIO)
...
async def test_top_muestra_ranking(cog):
    await registrar_servicio(5, 45, user_id=USUARIO, nombre="Tester")
    await registrar_servicio(8, 0, user_id=7, nombre="Otro")
    interaccion = InteraccionFalsa(GUILD, USUARIO)
...
async def test_ayuda_muestra_los_comandos(cog):
    interaccion = InteraccionFalsa(GUILD, USUARIO)
